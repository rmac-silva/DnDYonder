...
function SheetListings() {
    ...
    const getClassIcon = (className) => {
        ...
        switch (className.toLowerCase()) {
            case "artificer":
                return (<ArtificerIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "barbarian":
                return (<BarbarianIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "bard":
                return (<BardIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "cleric":
                return (<ClericIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "druid":
                return (<DruidIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "fighter":
                return (<FighterIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "monk":
                return (<MonkIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "paladin":
                return (<PaladinIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "ranger":
                return (<RangerIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "rogue":
                return (<RogueIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "sorcerer":
                return (<SorcererIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "warlock":
                return (<WarlockIcon className={`w-${iconSize} h-${iconSize}`} />)
            case "wizard":
                return (<WizardIcon className={`w-${iconSize} h-${iconSize}`} />)
        }
    }
    ...
    useEffect(() => {
        ...
        const fetchSheets = async () => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_URL}/sheets/${authUsername}`,
                    { method: "GET" }
                );
    ...
    function GetCharacterDisplayName(sheet) {
        ...
        return (
            <>
                <Typography variant="body1" className='!text-xl text-shadow-lg' component="div" align="center">
                    Lvl. {sheet.stats.level}
                </Typography>
                <Typography variant="body2" className='!text-xl text-shadow-lg' component="div" align="center">
...
export function WizardIconSimple(props) {
    return <i className="fa fa-magic" {...props}></i>;
}
